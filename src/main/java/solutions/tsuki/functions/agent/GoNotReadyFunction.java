...
@ApplicationScoped
public class GoNotReadyFunction implements Function<AgentRequest, AgentResponse> {
    public final Logger logger = LoggerFactory.getLogger("GoNotReady-Function");
    @Inject
    ...
    public AgentResponse apply(AgentRequest request) {
        ...
        boolean result = pgPool.preparedQuery(AGENT_QUERIES.INSERT_AGENT_STATE_CHANGE_EVENT)
                .execute(Tuple.of(agent.getId(), AGENT_STATES.NOT_READY, request.getRequestAt()))
                .onItemOrFailure().transform((item, throwable) -> {
                    if (throwable != null) {
                        logger.error("failed to insert agent [{}] not readt event into DB", agent.getId(), throwable);
                        return false;
                    } else {
                        return true;
                    }
        ...
        logger.info("agent [{}] state changed to not ready, not ready at time is [{}]", agent.getId(),
