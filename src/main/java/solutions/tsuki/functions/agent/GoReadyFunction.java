...
@ApplicationScoped
public class GoReadyFunction implements Function<AgentRequest, AgentResponse> {
    ...
    public AgentResponse apply(AgentRequest request) {
        ...
        boolean result = pgPool.preparedQuery(AGENT_QUERIES.INSERT_AGENT_STATE_CHANGE_EVENT)
                .execute(Tuple.of(agent.getId(), AGENT_STATES.READY, request.getRequestAt()))
                .onItemOrFailure().transform((item, throwable) -> {
                    if (throwable != null) {
                        logger.error("failed to insert agent [{}] ready event into DB", agent.getId(), throwable);
                        return false;
                    } else {
                        return true;
                    }
        ...
        queuesOfAgentsStore.sortedInsert(agent);
        queueOfQueuesOfInteractions.validateByAgent(agent);
        singleThreadExecutor.getExecutor().execute(() -> queueOfQueuesOfInteractions.process());
        logger.info("agent [{}] state changed to ready at [{}]", agent.getId(), agent.getTimeMeasurements().getIdleAt());
