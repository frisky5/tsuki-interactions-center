...
@ApplicationScoped
public class GoOfferingInteractionFunction implements Function<AgentRequest, AgentResponse> {
    ...
    public AgentResponse apply(AgentRequest request) {
        ...
        if (!allowed) {
            response.setError(true);
            response.setMessage("not allowed change state");
            return response;
        }

        boolean result = pgPool.preparedQuery(AGENT_QUERIES.INSERT_AGENT_STATE_CHANGE_EVENT)
                .execute(Tuple.of(agent.getId(), AGENT_STATES.OFFERING_AN_INTERACTION, request.getRequestAt()))
                .onItemOrFailure().transform((item, throwable) -> {
                    if (throwable != null) {
                        logger.error("failed to insert agent [{}] offering an interaction event into DB", agent.getId(),
                                throwable);
                        return false;
                    } else {
                        return true;
        ...
        if (!result) {
            response.setError(true);
            response.setCode(-1);
            response.setMessage("Failed to change agent state");
            return response;
        }
        ...
        logger.info("agent [{}] state changed to offering an interaction at [{}]", agent.getId(),
